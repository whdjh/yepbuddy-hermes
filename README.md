...
GitHub에는 코드만 저장합니다. `.env`, `config/topic_routes.json`, `data/*.jsonl`, `data/hermes_tokens.json`은 안드로이드 서버 로컬에만 둡니다.

## 현재 범위

- Telegram bot 연결 확인: `/ping`
- 내 Telegram user/topic 확인: `/whoami`, `/topicid`
- Hermes OAuth 시작: `/auth`
- Hermes OAuth code 저장: `/authcode <code>`
- Hermes에게 질문: `/ask <질문>`
...
현재 작업 폴더 이름은 나중에 바꿔도 됩니다. GitHub repo를 `hermes`로 새로 만들면 remote만 바꾸면 됩니다.
...
```bash
git remote set-url origin git@github.com:whdjh/hermes.git
```

아직 remote가 없다면:

```bash
git remote add origin git@github.com:whdjh/hermes.git
```
...
HTTPS clone을 쓸 거면 remote URL은 `https://github.com/whdjh/hermes.git`로 써도 됩니다.
...
pkg update
pkg install git
git clone https://github.com/whdjh/hermes.git
cd hermes
...
HERMES_AUTH_URL=https://...
HERMES_TOKEN_URL=https://...
HERMES_CLIENT_ID=...
HERMES_CLIENT_SECRET=...
HERMES_REDIRECT_URI=urn:ietf:wg:oauth:2.0:oob
HERMES_SCOPE=
...
## 5. Telegram group/topic 만들기

텔레그램용 핸드폰에서 새 group을 만들고 봇을 초대합니다.

그룹 설정에서 Topics 또는 Forum 기능을 켭니다. 테스트용 topic을 하나 만듭니다.
...
텔레그램용 핸드폰에서 group topic 안에 보냅니다.
...
Hermes가 로그인 URL을 보냅니다. 텔레그램용 핸드폰 브라우저에서 열고 로그인합니다.
...
성공하면 token이 안드로이드 서버의 `data/hermes_tokens.json`에 저장됩니다. 이 파일은 GitHub에 올라가지 않습니다.
...
Hermes API 응답이 오면 연결 완료입니다.

`/ask`가 실패하면 먼저 아래를 확인합니다.

- `.env`의 `HERMES_API_BASE_URL`
- `.env`의 `HERMES_CHAT_PATH`
- `.env`의 `HERMES_MODEL`
- `data/hermes_tokens.json` 존재 여부
- Termux 로그
...
```bash
ls data
cat data/hermes_tokens.json
cat data/role_events.jsonl
cat data/denied_updates.jsonl
```

`data/hermes_tokens.json`은 secret입니다. 캡처하거나 GitHub에 올리지 마십시오.

## 11. Mac에서 개발한 뒤 안드로이드 서버에 반영
...
- `/auth`: Hermes OAuth 로그인 URL 생성
- `/authcode <code>`: Hermes OAuth token 저장
- `/ask <질문>`: Hermes에게 질문
